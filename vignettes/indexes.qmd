...
get_measure_summary(grid_even, obs) |> calc_spi()
```
...
```{r}
get_measure_summary(grid_uneven, obs) |> calc_spi()
```
...
```{r}
get_measure_summary(grid_even, obs) |> 
  calc_ei() |> 
...
```{r}
get_measure_summary(grid_uneven, obs) |> 
  calc_ei() |> 
  dplyr::select(zone, ei) |> 
  gt::gt()
```
